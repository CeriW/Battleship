...
exports.placeShips =
  exports.checkValidShipState =
  exports.generateRandomPosition =
  exports.shipTypes =
  exports.initialiseShipArray =
    void 0;
var ai_behaviour_1 = require('../ai-behaviour');
function initialiseShipArray() {
  var array = [];
  for (var i = 0; i < 10; i++) {
    array[i] = new Array(10).fill({ name: '', status: null });
  }
  return array;
}
...
exports.checkValidShipState = function (_a) {
  var proposedPositions = _a.proposedPositions,
    shipSize = _a.shipSize,
    existingPositions = _a.existingPositions,
    _b = _a.adjacentShipModifier,
    adjacentShipModifier = _b === void 0 ? ai_behaviour_1.ai.adjacentShipModifier : _b;
      ...
      if (validShipState) {
        if (proposedPositions.alignment === 'horizontal') {
          for (var i = proposedPositions.startingColumn; i < proposedPositions.startingColumn + ship.size; i++) {
            positions[proposedPositions.startingRow][i] = { name: ship.name, status: false };
          }
        } else {
          // vertical placement
          for (var i = proposedPositions.startingRow; i < proposedPositions.startingRow + ship.size; i++) {
            positions[i][proposedPositions.startingColumn] = { name: ship.name, status: false };
          }
        }
      }
    }
